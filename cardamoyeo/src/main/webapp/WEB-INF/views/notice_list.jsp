				...
				<table class="table table-hover" id="dataTables-example">
					...
					<c:choose>
						<c:when test="${empty list}">
							<tr class="text-center"><td colspan ="5">---공지내용이 없습니다.---</td></tr>
						</c:when>
						<c:otherwise>
							<c:forEach items="${list}" var="dto" varStatus="status">
								<tr>
									<td>${status.count}</td>
									<td><a href="notice_detail?board_seq=${dto.board_seq}">${dto.board_title}</a></td>
									<td class="text-center">${dto.user_id}</td>
									<td class="text-center"><fmt:formatDate value="${dto.board_date}" pattern="yyyy-MM-dd"/> </td>
									<td class="text-center">${dto.read_count}</td>
								</tr>
							</c:forEach>
						</c:otherwise>
					</c:choose>
						...
						<tr>
							<td colspan="5" style="text-align: center;">
								<ul class="pagination" style="margin:0 auto;">					
									<a href="notice_list?pNum=<%=map.get("prePageNum")%>">◀</a>							
									<%
									for(int i=map.get("startPage") ;i<=map.get("endPage");i++){
									%>
											<li><a href="notice_list?pNum=<%=i%>" <%=pNum.equals(i+"")?"class='active' style='font-weight: bold;'":"" %>  ><%=i%></a></li>
											<%-- <li><a href="notice_list?pNum=<%=i%>" <%=pNum.equals(i+"")?" style='font-weight: bold;text-decoration: underline;'":"" %>  ><%=i%></a></li> --%>
									<%
									}
									%>
									<a href="notice_list?pNum=<%=map.get("nextPageNum")%>">▶</a>						
									<%-- 현재 페이지 : <li class="active"><a href="notice_list?pNum=<%=pNum%>"><b><%=pNum%></b></a></li> --%>
								</ul>
							</td>
						</tr>
					</table>
		...
		<sec:authorize access="isAuthenticated()">
			<sec:authorize access="hasAuthority('ROLE_M')">
				...
				<button id='writeBtn' type="button" class="btn btn-outline btn-primary">
					<a href="notice_write" class="text-white">글쓰기</a>					
				</button>
			</div>
			</sec:authorize>
		</sec:authorize>
