 ...
 <!-- NAVBAR -->
	...
	<section class="site-section">
		<div class="container">
			...
			<form name="deleteForm" action="qna_detail" method="post">
			...
			<!-- <button type="submit" data-oper="remove" class="btn btn-outline btn-primary"><a href="qna_view" class="text-white">글보기</a></button> -->
				<button type="submit" data-oper="list" class="btn btn-outline btn-primary"><a href="qna_list" class="text-white">목록보기</a></button>${msg}
			</form>
		</div>
	</section>
   ...
   <script>   
	$(function(){
		$("#frm").submit(function(){
			...
			if($("board_pass").val() != "${dto.board_pass}"){
				$("#board_pass").focus();
				alert("비밀번호가 다릅니다");
				return false;
			}
			else($("board_pass").val() == "${dto.board_pass}"){
				location.href="qna_view?seq=${dto.board_seq}"
			}
		});
	});
</script>
