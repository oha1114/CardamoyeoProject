...
%> 
						...
						<c:choose>
							...
							<c:otherwise>
								<c:forEach items="${list}" var="dto" varStatus="status">
									<tr>
										<td>${status.count}</td>
										<td><a href="qna_pass_owner?board_seq=${dto.board_seq}">${dto.board_title}</a><span style="color:red;"> (${dto.commentCount})</span></td>
							...
							<tr>
								<td colspan="5" style="text-align: center;">
									<ul class="pagination" style="margin:0 auto;">					
										<a href="qna_list_owner?pNum=<%=map.get("prePageNum")%>">◀</a>							
										<%
										...
										%>
												...
												<%-- <li><a href="qna_list_owner?pNum=<%=i%>" <%=pNum.equals(i+"")?"style='font-weight: bold;text-decoration: underline;'":"" %>  ><%=i%></a></li> --%>
										<%
										}
										%>
										<a href="qna_list_owner?pNum=<%=map.get("nextPageNum")%>">▶</a>						
									</ul>
								</td>
							</tr>	
						</table>
			...
			<div class="col-xs-6 text-right addbtn">
				...
				<button id='writeBtn' type="button" class="btn btn-outline btn-primary">
					<a href="qna_write_owner" class="text-white">글쓰기</a>								
				</button>
				</sec:authorize>
			</div>
       </div>
